...
const SELECT = `
  SELECT p.id, p.user_id, p.name, p.home_location_id, p.notes, p.created_at, p.updated_at,
...
export async function createPassenger(
  userId: string,
  data: { name: string; home_location_id: string; notes: string | null },
): Promise<Passenger> {
  const rows = await query<Record<string, unknown>>(
    `INSERT INTO passengers (user_id, name, home_location_id, notes)
     VALUES ($1, $2, $3, $4)
     RETURNING id`,
    [userId, data.name, data.home_location_id, data.notes],
  )
...
export async function updatePassenger(
  id: string,
  userId: string,
  data: { name: string; notes: string | null; home_location_id: string },
): Promise<Passenger | null> {
  const rows = await query<Record<string, unknown>>(
    `UPDATE passengers
     SET name = $1, notes = $2, home_location_id = $3, updated_at = now()
     WHERE id = $4 AND user_id = $5
     RETURNING id`,
    [data.name, data.notes, data.home_location_id, id, userId],
  )
