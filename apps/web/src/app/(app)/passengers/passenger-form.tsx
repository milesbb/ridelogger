...
export function PassengerForm({ existing, onDone }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (existing) {
        await api.passengers.update(existing.id, { name, homeAddress, notes })
      } else {
        await api.passengers.create({ name, homeAddress, notes })
      }
      onDone()
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Input id="home-address" value={homeAddress} onChange={(e) => setHomeAddress(e.target.value)} placeholder="123 Main St, Suburb VIC 3000" required />
      </div>
      <div className="space-y-2">
      ...
      <div className="flex gap-3 pt-2">
        <Button type="submit" disabled={loading}>{loading ? "Saving…" : existing ? "Save changes" : "Add passenger"}</Button>
